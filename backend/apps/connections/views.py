...
class DatabaseConnectionListCreateView(ListCreateAPIView):
    ...
    def get_queryset(self):
        return DatabaseConnection.objects.filter(application=self.get_application())
...
class DatabaseConnectionDetailView(RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        ...
        qs = DatabaseConnection.objects.select_related("application__organization")
...
class DatabaseConnectionTestView(APIView):
    ...
    def post(self, request, id):
        connection = get_object_or_404(
            DatabaseConnection.objects.select_related("application__organization"), id=id
        )
